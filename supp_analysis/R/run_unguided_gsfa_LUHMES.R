...
setwd("/project2/xinhe/kevinluo/GSFA/unguided_GSFA/LUHMES/")

print("Loading inputs for GSFA...")
# Processed gene expression matrix:
scaled.gene_exp <- readRDS("processed_data/deviance_residual.merged_top_6k.corrected_4.scaled.rds")
# Cell meta data:
metadata <- readRDS("processed_data/merged_metadata.rds")
# # Perturbation info:
# G_mat <- metadata[, 4:18]
...
if (!opt$permute){
  ...
  out_dir <- paste0(out_folder, "All.gibbs_obj_k", K, ".unguided.", out_suffix,
                    ".seed_", random_seed, ".rds")
  if (file.exists(out_dir)){
    warnings("Output file exists and will be overwritten!")
  }
} else {
  # Perform one of ten premuation tests
  ...
  out_dir <- paste0(out_folder, "All.gibbs_obj_k", K, ".unguided.", paste0("perm_", opt$perm_num), ".rds")
}
