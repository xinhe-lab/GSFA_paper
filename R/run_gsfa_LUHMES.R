...
## Results can be reproduced under default parameters with the output of
## "scaled.gene_exp" and "G_mat" from "preprocess_LUHMES.R" as GSFA input.
## Gibbs sampling is performed for 3000 iterations in one run,
...
## to make things easier if only limited run time and memory can be allocated.
## Three output files:
...
option_list <- list(
  ...
  make_option(
    "--lfsr_niter", action = "store", default = 1000, type = 'integer',
    help = "The number of iterations to estimate LFSR on, default is %default"
  ),
  make_option(
...
if (!opt$permute){
  ...
  out_dir <- paste0(out_folder, "gibbs_obj_k", K, ".", paste0("perm_", opt$perm_num), ".rds")
}
...
if (opt$restart == F){
  ...
  fit <- fit_gsfa_multivar(Y = scaled.gene_exp, G = G_mat, K = K,
                           ...
                           niter = niter, average_niter = average_niter, lfsr_niter = lfsr_niter,
                           verbose = T, return_samples = T, seed = random_seed)
} else {
...
lfsr_mat <- fit$lfsr
lfsr_mat <- lfsr_mat[, "offset"]
rownames(lfsr_mat) <- colnames(scaled.gene_exp)
print("# of genes that pass LFSR < 0.05 (computed from all factors):")
