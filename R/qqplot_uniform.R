# credit to Matthew Flickinger (https://genome.sph.umich.edu/wiki/Code_Sample:_Generating_QQ_Plots_in_R)
library(lattice)
qqunif.plot <- function(pvalues,
  ...
  if (is.list(pvalues)) {
    ...
    for (i in 1:length(pvo)) {
      if (!already.transformed) {
        ...
        exp.x[rs[i]:re[i]] <-
          -log10((rank(pvo[[i]], ties.method = "first") - .5) / nn[i])
      } else {
        pvalues[rs[i]:re[i]] <- pvo[[i]]
        exp.x[rs[i]:re[i]] <-
          -log10((nn[i] + 1 - rank(pvo[[i]], ties.method = "first") - .5) / (nn[i] + 1))
      }
    }
  } else {
    n <- length(pvalues) + 1
  ...
  # Helper function to draw the confidence interval.
  panel.qqconf <-
    function(n,
             conf.points = 1000,
             conf.col = "gray",
      ...
      for (i in seq(from = 1, to = conf.points)) {
        ...
        mpts[conf.points * 2 + 1 - i, 2] <-
          -log10(qbeta(conf.alpha / 2, i, n - i))
      }
      grid.polygon(
        x = mpts[, 1],
        y = mpts[, 2],
        gp = gpar(fill = conf.col, lty = 0),
