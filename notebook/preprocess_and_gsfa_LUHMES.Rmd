...
The study targeted 14 neurodevelopmental genes, including 13 autism risk genes, with CRISPR knock-down in Lund human mesencephalic (LUHMES) neural progenitor cells. The overall goal is to understand the effects of these target genes on the transcriptome and on the process of neuronal differentiation.
...
Numbers of cells and genes in the merged gene expression matrix:
```{r}
dim(combined_obj)
```
...
Numbers of cells and genes after QC:
```{r}
dim(combined_obj)
```
...
**Due to the size of the data, this process can take up to several hours, and should be run in an R script separately.**
...
The corrected and scaled gene expression matrix (8708 cells by 6000 genes) will be used as input for GSFA `fit_gsfa_multivar` argument $Y$. We annonate this matrix with cell and gene names so we can keep track.
...
In addition, we have a cell by perturbation matrix containing gene-level perturbation conditions across cells for GSFA `fit_gsfa_multivar` argument $G$:
...
Now that we have the two inputs we need, we can perform GSFA (guided sparse factor analysis) on the data.

Below we specify 20 factors initialized from truncated SVD and run Gibbs sampling for 3000 iterations, with the posterior mean estimates computed over the last 1000 iterations of samples.
...
More options for running GSFA are available in the `R/run_gsfa_LUHMES` script. For example, one can run Gibbs sampling in segments and restart from a previous run if limited computational resources are available.
